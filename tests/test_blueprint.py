...
def test_real_blueprint_selects_next_unfinished_milestone() -> None:
    ...
    assert status["current_phase"]["id"] == "P6"
    assert status["current_milestone"]["id"] == "P6.M3"
    assert "continue" in render_blueprint_status(blueprint)
    assert any("process-reward GRPO task metadata" in action for action in status["next_actions"])
