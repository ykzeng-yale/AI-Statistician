...
Coordinate edits to these paths because P12.M2 has synchronized them and P12.M3
will continue from the endpoint-assembly layer:
...
## Immediate Promotion Checklist For Active P12.M3

P12.M2 is promoted.  Before promoting P12.M3:

- build endpoint strong-law assembly signatures for selected finite bracket
  endpoints generated by the proof-carrying `L1(P)` bracketing-number witness;
- keep endpoint measurability, finite-norm, and sample-average semantics
  explicit instead of folding them into the bracketing-number witness;
- add theorem-hole seeds for endpoint-to-constructor assembly only after the
  endpoint signatures compile;
