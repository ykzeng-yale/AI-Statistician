...
def test_real_blueprint_selects_next_unfinished_milestone() -> None:
    ...
    assert status["current_phase"]["id"] == "P10"
    assert status["current_phase"]["status"] == "pending"
    assert status["current_milestone"]["id"] == "P10.M1"
    assert "continue" in render_blueprint_status(blueprint)
    assert any("bracketing" in action.lower() for action in status["next_actions"])
