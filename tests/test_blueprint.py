...
def test_real_blueprint_selects_next_unfinished_milestone() -> None:
    ...
    assert status["current_phase"]["id"] == "P5"
    assert status["current_milestone"]["id"] == "P5.M2"
    assert "continue" in render_blueprint_status(blueprint)
    assert any("Regenerate verifier artifacts" in action for action in status["next_actions"])
