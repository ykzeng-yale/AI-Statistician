...
def test_real_blueprint_selects_next_unfinished_milestone() -> None:
    ...
    assert status["current_phase"]["id"] == "P9"
    assert status["current_phase"]["status"] == "in_progress"
    assert status["current_milestone"]["id"] == "P9.M2"
    assert "continue" in render_blueprint_status(blueprint)
    assert any("theorem-hole lemma promotion" in action.lower() for action in status["next_actions"])
